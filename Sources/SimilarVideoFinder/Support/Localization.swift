...
enum L10n {
    ...
    static func displayThresholdHelp(_ l: AppLanguage) -> String {
        let recommendedPercent = DisplayThresholdEditing.text(for: DisplayThresholdEditing.recommendedThreshold)
        return text(l,
                    "We recommend starting at \(recommendedPercent)%. Lowering the threshold shows more candidates. The score does not represent the probability that files are duplicates.",
                    "建议从 \(recommendedPercent)% 开始；降低阈值会显示更多候选。分数不代表重复概率。",
                    "建議從 \(recommendedPercent)% 開始；降低閾值會顯示更多候選。分數不代表重複機率。",
                    "Recomendamos empezar en \(recommendedPercent) %. Reducir el umbral muestra más candidatos. La puntuación no representa la probabilidad de que los archivos sean duplicados.",
                    "Nous recommandons de commencer à \(recommendedPercent) %. Abaisser le seuil affiche davantage de candidats. Le score ne représente pas la probabilité que les fichiers soient des doublons.",
                    "\(recommendedPercent)% から始めることをお勧めします。しきい値を下げると、より多くの候補が表示されます。スコアはファイルが重複している確率を表すものではありません。",
                    "\(recommendedPercent)%에서 시작하는 것을 권장합니다. 임계값을 낮추면 더 많은 후보가 표시됩니다. 점수는 파일이 중복일 확률을 나타내지 않습니다.")
    }
    ...
    static func deepVerificationHelp(_ l: AppLanguage) -> String { text(l, "Enables frame comparison for video candidates that are not exact duplicates. The first scan may take longer. Applies to the next scan.", "对非完全相同的视频候选启用画面比对。首次扫描可能更慢，对下一次扫描生效。", "對非完全相同的影片候選啟用畫面比對。首次掃描可能較慢，於下一次掃描生效。", "Activa la comparación de fotogramas para vídeos candidatos que no son duplicados exactos. El primer análisis puede tardar más. Se aplica al próximo análisis.", "Active la comparaison des images pour les vidéos candidates qui ne sont pas des doublons exacts. La première analyse peut prendre plus de temps. S’applique à la prochaine analyse.", "完全に同一ではない動画候補のフレーム比較を有効にします。初回スキャンには時間がかかる場合があります。次回のスキャンから適用されます。", "완전히 동일하지 않은 동영상 후보의 프레임 비교를 활성화합니다. 첫 스캔은 더 오래 걸릴 수 있습니다. 다음 스캔부터 적용됩니다.") }
